...
## agent_communication:
    ...
    -agent: "user"
    -message: "Reporte de bugs: 1) No puedo ver mi slot de anuncio en mi pulpería. 2) A veces cuando estoy loggeado en mi cuenta de pulpería, en el panel de abajo me sale un botón que me lleva al mapa, cuando solo debería ir al Dashboard."
    -agent: "main"
    -message: "Investigando bugs reportados. Problemas identificados: 1) BottomNav puede mostrar botón incorrecto si user.user_type no está correctamente definido, 2) Sistema de slots de anuncio global funcionando pero UI puede mejorar visualización."
    -agent: "main"
    -message: "Bugs corregidos: 1) BottomNav ahora detecta correctamente user_type con valor predeterminado, 2) Slot de anuncio solo se muestra cuando has_slot es true, 3) GlobalAnnouncementsPage ahora obtiene user correctamente. Archivos modificados: PulperiaDashboard.js, FeaturedAdsPage.js, BottomNav.js, GlobalAnnouncementsPage.js"
    -agent: "user"
    -message: "Nuevo bug: Al crear pulpería, sale pantalla roja con errores, pero después de refresh la pulpería sí existe."
    -agent: "main"
    -message: "Bug identificado y corregido. Problema: fetchData() después de crear pulpería causaba errores de estado. Solución: Usar window.location.reload() después de crear exitosamente para evitar problemas de sincronización de estado. También mejorado manejo de errores en fetchData para no mostrar toasts innecesarios."
    -agent: "user"
    -message: "Nuevo requerimiento: 1) Eliminar recuadro completo de logros del Dashboard. 2) Cambiar 'Historial' a 'Reportes' en BottomNav. 3) Integrar Reportes y Tips IA en página de Reportes. 4) Integrar botón compartir con link personalizado."
    -agent: "main"
    -message: "Implementados todos los cambios. Eliminado recuadro de logros del Dashboard, cambiado label de Historial a Reportes, agregada pestaña Tips IA en OrderHistory, integrados botones de redes sociales (WhatsApp, Facebook, Twitter) con link personalizado."
    -agent: "user"
    ...
    -agent: "user"
    -message: "Problema persiste: Si hago refresh en Dashboard y luego navego a otra pestaña (Chamba), me saca al inicio."
    -agent: "main"
    -message: "Problema de sesión completamente resuelto: 1) CACHÉ DE USUARIO: Implementado sistema de caché en localStorage con 'cached_user' que persiste el estado del usuario entre navegaciones y refreshes. 2) PROTECTEDROUTE MEJORADO: Simplificada lógica para verificar hasStoredToken en cada render (no solo en mount), mejor manejo de estados intermedios. 3) AUTHCONTEXT OPTIMIZADO: Usuario se inicializa desde caché al montar, todas las funciones (login, setUserType, etc) actualizan el caché, logout limpia ambos token y caché. 4) Eliminado useEffect de hasAttemptedAuth que causaba problemas. La sesión ahora persiste completamente durante navegación, refresh, y cambio de tabs. Frontend reiniciado."